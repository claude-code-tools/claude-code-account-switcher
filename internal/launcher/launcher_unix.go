...
// isolated to its own config dir. On Unix it replaces the current process so
// Claude owns the terminal directly.
package launcher
...
// Launch execs claude as the given account. It does not return on success.
func Launch(acct registry.Account, args []string) error {
	...
	if token == "" {
		fmt.Fprintf(os.Stderr, "No Keychain token found for %s.\nRun: claude-accounts\n", acct.Label)
	...
	return syscall.Exec(bin, argv, buildEnv(token, acct, configDir))
}
